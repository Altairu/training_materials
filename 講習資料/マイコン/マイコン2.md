...
void loop() {
  // 指定した距離と方向に移動する
  moveRobot(100.0, 0.0, 5.0); // 100 mm 前進、0 度の回転、5秒かけて移動
  delay(2000); // 2秒待機
...
void moveRobot(float distance, float angle, float time) {
  ...
  kinematics.computeWheelSpeeds(linearVelocity, angularVelocity, leftSpeed, rightSpeed);

  long targetPulsesDistance = (distance / (PI * wheelDiameter)) * pulsesPerRevolution;
  long targetPulsesAngle = (angle / 360.0) * (wheelBase / (PI * wheelDiameter)) * pulsesPerRevolution;
  ...
  while (abs(leftEncoder.getPulses()) < abs(targetPulsesDistance) && abs(rightEncoder.getPulses()) < abs(targetPulsesAngle)) {
    // エンコーダのカウントを監視しながら移動
  }
...
## 説明
1. `moveRobot`関数で、指定した距離（mm）、角度（度）、および移動時間（秒）を入力します。
2. 指定した時間に基づいて線速度（mm/s）と角速度（rad/s）を計算します。
3. `Kinematics`ライブラリを使用して、これらの速度から左車輪と右車輪の速度（RPM）を計算します。
4. エンコーダのパルス数に基づいて、ロボットが目標距離と角度に到達するまでモーターの速度を調整します。
5. ロボットが目標に到達したら、モーターを停止します。
